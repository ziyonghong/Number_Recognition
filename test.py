...
cropImg =origin[200:700,600:1500] #截取区域
plt.subplot(2, 1, 1)
#plt.imshow(origin)
plt.imshow(cropImg)
#ret, image = cv2.threshold(origin, 100, 255, cv2.THRESH_BINARY)  # 第一个数字是二值化的阈值，要多尝试，255就不要变了
ret, image = cv2.threshold(cropImg, 100, 255, cv2.THRESH_BINARY)
plt.subplot(2, 1, 2)
plt.imshow(image)
